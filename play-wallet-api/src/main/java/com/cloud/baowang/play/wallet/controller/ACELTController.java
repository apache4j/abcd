...
@RestController
@AllArgsConstructor
@RequestMapping("/acelt/rest/freetransfer")
@Tag(name = "彩票回调")
public class ACELTController {

//    private final ACELTService aceltService;

    private final ACELTGameApi aceltGameApi;
    ...
    public ACELTBaseRes<ACELTGetBalanceRes> getUserBalance(@RequestBody ACELTGetBalanceReq request) {
        ...
        log.info("收到{}-查询免转钱包用户余额 消息:{}", VenueEnum.ACELT.getVenueName(), JSON.toJSON(request));
        ACELTBaseRes<ACELTGetBalanceRes> result = aceltGameApi.queryBalance(request);
        long endTime = System.currentTimeMillis();
        long durationSeconds = (endTime - startTime) / 1000;
        log.info("收到{}-查询免转钱包用户余额 ,执行了:{}秒, 消息:{}", VenueEnum.ACELT.getVenueName(), durationSeconds, request);
        return result;
    }
    ...
    public ACELTBaseRes<ACELTAccountCheangeRes> accountChange(@RequestBody ACELTAccountCheangeReq request) {
        long startTime = System.currentTimeMillis();
        log.info("收到{}-账变接口 消息:{}", VenueEnum.ACELT.getVenueName(), JSON.toJSON(request));
        ACELTBaseRes<ACELTAccountCheangeRes> result = aceltGameApi.accountChange(request);
        long endTime = System.currentTimeMillis();
        long durationSeconds = (endTime - startTime) / 1000;
        log.info("收到{}-账变接口 ,执行了:{}秒, 消息:{}", VenueEnum.ACELT.getVenueName(), durationSeconds, request);
        return result;
    }
    ...
    public ACELTBaseRes<ACELTAccountCheangeRes> accountChangeCallBack(@RequestBody ACELTAccountChangeCallBackReq request) {
        long startTime = System.currentTimeMillis();
        log.info("收到{}-免转钱包帐变回调接口 消息:{}", VenueEnum.ACELT.getVenueName(), JSON.toJSON(request));
        ACELTBaseRes<ACELTAccountCheangeRes> result = aceltGameApi.accountChangeCallBack(request);
        long endTime = System.currentTimeMillis();
        long durationSeconds = (endTime - startTime) / 1000;
        log.info("收到{}-免转钱包帐变回调接口 ,执行了:{}秒, 消息:{}", VenueEnum.ACELT.getVenueName(), durationSeconds, request);
        return result;
    }
