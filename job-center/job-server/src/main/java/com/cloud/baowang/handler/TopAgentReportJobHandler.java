...
@Component
@Slf4j
public class TopAgentReportJobHandler {

    @Resource
    private ReportTopAgentStaticsApi reportTopAgentStaticsApi;

    @Resource
    private SiteApi siteApi;

    /**
     * 按天统计商务总代报表 频率 每小时执行一次
     ...
     */
    @XxlJob(value = "topAgentReportDay")
    public void agentMerchantReportDay() {
        ...
        if(StringUtils.hasText(siteCode)){
            ...
            for(SiteVO siteVO:siteVOList){
                ...
                if(immediateFlag){
                    log.info("站点:{},历史数据,商务总代报表开始初始化",siteVO.getSiteCode());
                    matchSiteList.add(siteVO);
                }else {
                    log.info("站点:{},商务总代报表初始化,当前时间:{},当前小时:{}",siteVO.getSiteCode(),nowByTimeZoneStr,hourStr);
                    if(hourStr.equals("00")){
                        log.info("站点:{},符合条件,商务总代报表开始初始化",siteVO.getSiteCode());
                        matchSiteList.add(siteVO);
                    }
                }
            }
        ...
        if(!CollectionUtils.isEmpty(matchSiteList)){
            for(SiteVO siteVO:matchSiteList){
                ReportTopAgentStaticsCondVO reportTopAgentStaticsCondVO=new ReportTopAgentStaticsCondVO();
                reportTopAgentStaticsCondVO.setSiteCode(siteVO.getSiteCode());
                reportTopAgentStaticsCondVO.setReportType("0");
                reportTopAgentStaticsCondVO.setTimeZone(siteVO.getTimezone());
               // reportTopAgentStaticsCondVO.setStartDayMillis(1733500800000L);
                //reportTopAgentStaticsCondVO.setEndDayMillis(DateUtils.getYesTodayStartTime(siteVO.getTimezone()));
                if(immediateFlag){
                    if(paramJson.containsKey("beginDate")){
                        reportTopAgentStaticsCondVO.setStartDayMillis(paramJson.getLong("beginDate"));
                    }
                    if(paramJson.containsKey("endDate")){
                        reportTopAgentStaticsCondVO.setEndDayMillis(paramJson.getLong("endDate"));
                    }
                    log.info("站点:{},按天开始初始化商务总代报表:{}",siteVO.getSiteCode(),reportTopAgentStaticsCondVO);
                }else {
                    reportTopAgentStaticsCondVO.setStartDayMillis(DateUtils.getYesTodayStartTime(siteVO.getTimezone()));
                    reportTopAgentStaticsCondVO.setEndDayMillis(DateUtils.getYesTodayEndTime(siteVO.getTimezone()));
                    log.info("站点:{},开始初始化昨日商务总代报表:{}",siteVO.getSiteCode(),reportTopAgentStaticsCondVO);
                }
                reportTopAgentStaticsApi.init(reportTopAgentStaticsCondVO);
                log.info("站点:{},按天初始化商务总代报表完成:{}",siteVO.getSiteCode(),reportTopAgentStaticsCondVO);
            }
        }
    }
