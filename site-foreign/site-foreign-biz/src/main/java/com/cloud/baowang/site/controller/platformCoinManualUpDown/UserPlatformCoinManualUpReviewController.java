 ...
 * @author: qiqi
 */
@Tag(name = "资金-资金审核-会员平台币上分审核")
@AllArgsConstructor
@RestController
@RequestMapping("/user-platform-coin-manual-up-review/api")
public class UserPlatformCoinManualUpReviewController {
    ...
    public ResponseVO<Boolean> lock(@Valid @RequestBody StatusListVO vo) {
        return userPlatformCoinManualUpReviewApi.lock(vo, CurrReqUtils.getOneId(), CurrReqUtils.getAccount());
    }
    ...
    public ResponseVO<Boolean> oneReviewSuccess(@Valid @RequestBody WalletReviewListVO vo) {
        return userPlatformCoinManualUpReviewApi.oneReviewSuccess(vo, CurrReqUtils.getOneId(), CurrReqUtils.getAccount());
    }
    ...
    public ResponseVO<Boolean> oneReviewFail(@Valid @RequestBody WalletReviewListVO vo) {
        return userPlatformCoinManualUpReviewApi.oneReviewFail(vo, CurrReqUtils.getOneId(), CurrReqUtils.getAccount());
    }

    @Operation(summary = "审核列表")
    @PostMapping(value = "/getUpReviewPage")
    public ResponseVO<Page<UserPlatformCoinManualUpReviewResponseVO>> getUpReviewPage(@Valid @RequestBody UserPlatformCoinManualUpReviewPageVO vo) {
        vo.setSiteCode(CurrReqUtils.getSiteCode());
        vo.setOperator(CurrReqUtils.getAccount());
        return ResponseVO.success(userPlatformCoinManualUpReviewApi.getUpReviewPage(vo, CurrReqUtils.getAccount()));
    }

    @Operation(summary = "审核详情")
    @PostMapping(value = "/getUpReviewDetails")
    public ResponseVO<UserPlatformCoinUpReviewDetailsVO> getUpReviewDetails(@Valid @RequestBody IdVO vo) {
        vo.setSiteCode(CurrReqUtils.getSiteCode());
