 ...
 */
package com.cloud.baowang.play.task.pulltask.db.acelt;
 ...
 */
@Log4j2
@Component(ThirdGamePullBetTaskTypeConstant.DB_ACELT_GAME_PULL_BET_TASK)
@AllArgsConstructor
public class DBAceltGamePullBetTask extends BasePullBetTask {

    private final DBAceltServiceImpl aceltService;

    /**
     * 最多支持5分钟
     */
    public static final Long DEFAULT_STEP = 60 * 1000L;
    ...
    protected VenuePullBetParams pullBetRecord(VenueInfoVO venueDetailVO, String pullParamJson) {
        ...
        log.info("db彩票拉单开始: {}, 类型: {}", pullParamJson, pullType ? "自动" : "手动");
        ...
        while (true) {
            ...
            // 调用拉单接口
            ResponseVO<?> responseVO = aceltService.getBetRecordList(venueDetailVO, venuePullParamVO);
            if (!responseVO.isOk()) {
    ...
    protected String getVenuePlatform() {
        return VenuePlatformConstants.DBACELT;
    }
    ...
    protected VenuePullBetParams initPullParams() {
        ...
        log.info("db彩票_拉单-auto : "+venuePullBetParams);
        return venuePullBetParams;
    }
    ...
    protected VenuePullBetParams genNextPullParams(VenuePullBetParams currPullBetParams) {
        ...
        log.info("db彩票-下一次拉单参数 : "+newPullBetParams);
