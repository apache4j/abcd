 ...
 */
package com.cloud.baowang.play.task.pulltask.ftg.task;
 ...
 */
@Log4j2
@Component(ThirdGamePullBetTaskTypeConstant.FTG_GAME_PULL_BET_TASK)
@AllArgsConstructor
public class FTGGamePullBetTask extends BasePullBetTask {

    private final FTGGameServiceImpl ftgGameService;
    ...
    protected VenuePullBetParams pullBetRecord(VenueInfoVO venueDetailVO, String pullParamJson) {
        ...
        while (pullType) {
            ...
            i++;
            log.info("FTG拉单次数:{},开始时间start：{}，end：{}", i, venuePullBetParams.getStartTime(), venuePullBetParams.getEndTime());
            ...
            ResponseVO<?> responseVO = ftgGameService.getBetRecordList(venueDetailVO, venuePullParamVO);
            log.info("FTG拉单次数:{},结束", i);
            if(!responseVO.isOk()){
    ...
    protected String getVenuePlatform() {
        return VenuePlatformConstants.FTG;
    }
    ...
    protected VenuePullBetParams genNextPullParams(VenuePullBetParams currPullBetParams) {
        ...
        long timeInterval = params.getStep() == null ? (5 * 60 * 1000) : params.getStep();
