...
public enum FSResCodeEnum {
    ...
    public static FSResCodeEnum fromCode(String code) {
        for (FSResCodeEnum c : FSResCodeEnum.values()) {
    ...
    // 抽象方法：每个枚举值实现自己的返回逻辑
    public FSBaseRes toResVO(FSBaseRes resVO){
    ...
    // 抽象方法：每个枚举值实现自己的返回逻辑
    public FSBaseRes toResVO(){
        FSBaseRes resVO = new FSBaseRes();
        resVO.setCode(this.getCode());
        resVO.setMsg(this.getMsgEn());
        return resVO;
    }
