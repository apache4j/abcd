...
@Slf4j
@Service
public class SBABetActionConfirm extends SBABaseConfirm implements SBASportInterface {


    public SBABetActionConfirm(TransferRecordService transferRecordService) {
        super(transferRecordService);
    }

    @Override
    public SBActionEnum getAction() {
        return SBActionEnum.CONFIRM_BET;
    }
    ...
    public SBResBaseVO toAction(SBBaseReq baseReq) {
        SBConfirmBetReq req = JSONObject.parseObject(baseReq.getMessage(), SBConfirmBetReq.class);
        ...
        if (!req.validate()) {
            ...
            log.error("{} 上下分 参数异常,缺少参数:{}", getAction().getName(), req);
            return result;
        }

        UserInfoVO userInfoVOS = getUserInfo(req.getUserId());
        if (ObjectUtils.isEmpty(userInfoVOS)) {
            log.info("{} confirmBet 用户不存在 :{}", getAction().getName(),req.getUserId());
        ...
        for (SBTicketInfoReq item : req.getTxns()) {
            ...
            SBToCoinAddVO sbToCoinAddVO = SBToCoinAddVO.builder()
                    ...
                    .addAmount(item.getCreditAmount())
                    .subAmount(item.getDebitAmount())
                    .remark(getAction().getName())
                    .build();
            SBResultCode resultCode = toConfirmToCoin(SBActionEnum.CONFIRM_BET,sbToCoinAddVO, transferRecordResultVO);
