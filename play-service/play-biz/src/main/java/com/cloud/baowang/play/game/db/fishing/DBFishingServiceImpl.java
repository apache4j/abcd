package com.cloud.baowang.play.game.db.fishing;
...
@Service(ServiceType.GAME_THIRD_API_SERVICE + VenuePlatformConstants.DB_FISHING)
public class DBFishingServiceImpl extends GameBaseService implements GameService {
    ...
    private FishingBalanceChangeBody decrypt(String body,String merchantNo) {
        String key = cryptoConfig.getKey(VenueEnum.DBFISHING.getVenueCode(),merchantNo);
        String iv = cryptoConfig.getIV(VenueEnum.DBFISHING.getVenueCode(),merchantNo);
        try {
            String decrypt = AESDBUtil.decrypt(body, key, iv);
            return JSON.parseObject(decrypt, FishingBalanceChangeBody.class);
        } catch (Exception e) {
            log.error(" db_fishing 数据解密失败 - {}", e.getMessage());
            return null;
        }
    }
    ...
    public DBEVGBaseRsp checkRequestValid(UserInfoVO userInfoVO) {
        if (venueMaintainClosed( VenueEnum.DBFISHING.getVenueCode(),userInfoVO.getSiteCode())) {
        ...
        if (userGameLock(userInfoVO)) {
            log.error("玩家锁定{} - 场馆 : {}", userInfoVO.getUserName(), VenueEnum.DBFISHING.getVenueCode());
    ...
    public DBEVGBaseRsp queryBalance(DBEVGBasicInfo evgBasicInfo, String reqStr) {
        ...
        log.info("queryBalance : db_fishing - {}", req);
    ...
    public DBEVGBaseRsp balanceChange(DBEVGBasicInfo evgBasicInfo, String reqStr) {
        ...
        log.info("balanceChange : db_fishing - {}", req);
        UserInfoVO userInfoVO = userCheck(req);
        if (userInfoVO == null) {
            return DBEVGBaseRsp.failed(DBEVGErrorEnum.PARAM_ERROR);
        }
    ...
    public DBEVGBaseRsp queryOrderStatus(DBEVGBasicInfo evgBasicInfo, String reqStr) {
        ...
        log.info("queryOrderStatus : db_fishing - {}", req);
        UserInfoVO userInfoVO = userCheck(req);
        if (userInfoVO == null) {
            return DBEVGBaseRsp.failed(DBEVGErrorEnum.PARAM_ERROR);
        }
        ...
        casinoMember.setVenueCode(VenueEnum.DBFISHING.getVenueCode());
    ...
    private DBEVGBaseRsp handleSettle(FishingBalanceChangeBody req, UserInfoVO userInfoVO) {
        log.info("handleSettle : db_fishing - {}", req);
        ...
        if (coinRecordResultVO != null && UpdateBalanceStatusEnums.SUCCESS.equals(coinRecordResultVO.getResultStatus())) {
            ...
            log.info("db_fishing settle : 失败 : coinRecordResultVO -> " + coinRecordResultVO);
    ...
    private DBEVGBaseRsp handleBet(FishingBalanceChangeBody req, UserInfoVO userInfoVO) {
        ...
        String tradeId = req.getOrderId();//流水号
        String remark = getRemark(tradeId);
        CoinRecordResultVO coinRecordResultVO = handleBetCoin(userInfoVO, tradeId, tradeAmount, remark);
        if (coinRecordResultVO != null && UpdateBalanceStatusEnums.SUCCESS.equals(coinRecordResultVO.getResultStatus())) {
            BigDecimal balance = coinRecordResultVO.getCoinAfterBalance().setScale(2, RoundingMode.DOWN);
            CommonTradeInfo rsp = CommonTradeInfo.builder()
                    .tradeType(tradeType)
            ...
            log.info("db_fishing bet : 失败 : coinRecordResultVO -> " + coinRecordResultVO);
            return DBEVGBaseRsp.failed(DBEVGErrorEnum.SYSTEM_MAINTAINED);
        }
    }

    private String getRemark(String tradeId) {
        String remark = VenueEnum.DBFISHING.getVenueName();
    ...
    protected CoinRecordResultVO handleSettleCoin(UserInfoVO userInfoVO, String orderNo, BigDecimal payoutAmount, String remark) {
        ...
        userCoinAddVO.setVenueCode(VenueEnum.DBFISHING.getVenueCode());
    ...
    public ResponseVO<GameLoginVo> login(LoginVO loginVO, VenueInfoVO venueDetailVO, CasinoMemberVO casinoMemberVO) {
        ...
        dataMap.put("gameId", 200);
        String json = JSONObject.toJSONString(dataMap);
        log.info("DB_fishing三方登录参数 : " + json);
    ...
    private void handleRemoteOrder(List<DBOrderCommonVO> orderList, VenueInfoVO venueInfoVO) {
        ...
        //vip返水配置
        Map<String, String> rebateMap = getVIPRebateSingleVOMap(VenueEnum.DBFISHING.getVenueCode());

        //站点信息
        Map<String, String> siteNameMap = getSiteNameMap();

        //游戏信息
        Map<String, GameInfoPO> paramToGameInfo = getGameInfoByVenueCode(VenueEnum.DBFISHING.getVenueCode());
        ...
        for (DBOrderCommonVO order : orderList) {
            ...
            recordVO.setVenueType(VenueEnum.DBFISHING.getType().getCode());
            list.add(recordVO);
        }
        // 订单处理
...
//        }
    ...
    public static void loginTest() {
        String agent = "wintocnyby23e4dfda";
        String key = "np3q1Oa32mElLwyw";
