 ...
 */
package com.cloud.baowang.play.game.dbDj.task;
 ...
 */
@Log4j2
@Component(ThirdGamePullBetTaskTypeConstant.DB_DJ_GAME_PULL_BET_TASK)
@AllArgsConstructor
public class DbDJGamePullBetTask extends BasePullBetTask {

    private final DbDJGameServiceImpl dbDJGameService;
    ...
    protected VenuePullBetParams pullBetRecord(VenueInfoVO venueDetailVO, String pullParamJson) {
        ...
        while (pullType) {
            ...
            i++;
            log.info("DB电竞拉单次数:{},开始时间start：{}，end：{}", i, venuePullBetParams.getStartTime(), venuePullBetParams.getEndTime());
            ...
            ResponseVO<?> responseVO = dbDJGameService.getBetRecordList(venueDetailVO, venuePullParamVO);
            log.info("DB电竞拉单次数:{},结束", i);
            ...
            if (venuePullBetParams.getEndTime() >= System.currentTimeMillis() / 1000 - 5 * 60) {
                return venuePullBetParams;
            }
    ...
    protected VenuePullBetParams initPullParams() {
        ...
        long nowStart = System.currentTimeMillis() / 1000;
        long startTime = nowStart - (DbDJConstant.DEFAULT_STEP + 5 * 60); // 当前系统时间 - 30分钟 + 5分钟 = 开始时间

        //开始时间 + 29分钟后为结束时间
        long endTime = startTime + DbDJConstant.DEFAULT_STEP;
    ...
    protected VenuePullBetParams genNextPullParams(VenuePullBetParams currPullBetParams) {
        ...
        if(endTime > now / 1000 - 5 * 60){
            endTime = now / 1000 - 5 * 60;
        }
