 ...
 */
package com.cloud.baowang.play.task.pulltask.db.sh;
 ...
 */
@Log4j2
@Component(ThirdGamePullBetTaskTypeConstant.DB_SH_GAME_PULL_BET_TASK)
@AllArgsConstructor
public class DBSHGamePullBetTask extends BasePullBetTask {

    private final DBSHServiceImpl shService;
    ...
    protected VenuePullBetParams pullBetRecord(VenueInfoVO venueDetailVO, String pullParamJson) {
        ...
        log.info("dbsh 拉单开始: {}, 类型: {}", pullParamJson, pullType ? "自动" : "手动");
        ...
        while (true) {
            ...
            // 调用拉单接口
            ResponseVO<?> responseVO = shService.getBetRecordList(venueDetailVO, venuePullParamVO);
            if (!responseVO.isOk()) {
    ...
    protected VenuePullBetParams initPullParams() {
        ...
        log.info("dbsh_拉单-auto : "+venuePullBetParams);
        return venuePullBetParams;
    }
    @Override
    protected VenuePullBetParams genNextPullParams(VenuePullBetParams currPullBetParams) {
        ...
        log.info("dbsh-下一次拉单参数 : "+newPullBetParams);
