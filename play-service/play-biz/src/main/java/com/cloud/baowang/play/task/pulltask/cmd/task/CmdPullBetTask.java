package com.cloud.baowang.play.task.pulltask.cmd.task;
 ...
 */
@Slf4j
@Component(ThirdGamePullBetTaskTypeConstant.CMD_GAME_PULL_BET_TASK)
public class CmdPullBetTask extends BasePullBetTask {
    ...
    protected VenuePullBetParams pullBetRecord(VenueInfoVO venueInfoVO, String pullParamJson) {
        log.info("CMD拉单服务开始pullBetRecord, 场馆信息: {} 入参 : {}", venueInfoVO, pullParamJson);
        CmdPullBetParams venuePullBetParams = JSON.parseObject(pullParamJson, CmdPullBetParams.class);
        ResponseVO<CmdPullBetParams> responseVO = cmdServiceImpl.getBetRecordListByParams(venueInfoVO, venuePullBetParams);
        if (responseVO.isOk()) {
            log.info("CMD拉单服务结束pullBetRecord, 返回信息: {}", responseVO);
    ...
    protected VenuePullBetParams initPullParams() {
        ...
        params.setStarttime(TimeZoneUtils.formatDate4TimeZone(canadaCurrentDate, gmt8TimeZone, DateUtils.pattenT));
        params.setTimeInterval(CQ9Constant.DEFAULT_TIME_INTERVAL);
        // 按时间间隔计算结束时间
        //查詢日期開始時間，格式為RFC3339 如 20170601T000000
        //※時區請用UTC+8
        Date startAt = TimeZoneUtils.parseDate4TimeZoneCQ9(params.getStarttime(), gmt8TimeZone, DateUtils.pattenT);
        //查詢日期結束時間，格式為RFC3339 如 20170602T000000
        //※時區請用UTC+8
        Date endAt = new DateTime(startAt).plusMillis(params.getTimeInterval()).toDate();
        params.setEndtime(TimeZoneUtils.formatDate4TimeZone(endAt, gmt8TimeZone, DateUtils.pattenT));
        return params;
    }
    ...
    protected VenuePullBetParams initPullParams(String startTime) {
        // 时间转换
        CmdPullBetParams params = new CmdPullBetParams();
        String tarTime = TimeZoneUtils.formatLocalDateTime(TimeZoneUtils.timeByTimeZone(Long.parseLong(startTime),"UTC+8"), DateUtils.pattenT);
        // 设置开启与截止时间
        params.setStarttime(tarTime);
        params.setEndtime(tarTime);
        params.setTimeInterval(CmdConstant.DEFAULT_TIME_INTERVAL);

        // 获取下次拉单参数
        CmdPullBetParams CMDPullBetParams = cmdServiceImpl.nextPullBetParams(params);
        return CMDPullBetParams;
    }
}
