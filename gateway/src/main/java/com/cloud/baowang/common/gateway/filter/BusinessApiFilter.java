...
import com.cloud.baowang.common.gateway.properties.BusinessIgnoreAuthProperties;
...
@Component
@Slf4j
public class BusinessApiFilter implements BusinessAbstractFilter {
    private final BusinessIgnoreAuthProperties ignoreAuth;
    private final AuthenticationProperties authenticationProperties;


    public BusinessApiFilter(BusinessIgnoreAuthProperties ignoreAuth,
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        log.info("商务后台接口获取到的xCustom:{}", xCustom);
        ...
        if (!StringUtils.hasText(siteCodeAndTimezone)) {
            log.info("xCustom: 商务后台接口获取到的siteCode与timezone:{}为空", siteCodeAndTimezone);
        ...
        BusinessTokenVO businessTokenVO = BusinessAuthUtil.agentAuth(token);
       ...
       /* if (!siteCode.equals(siteTokenVO.getSiteCode())) {
        ...
        // 设置用户信息到请求
        addHeader(mutate, RequestBasicInfo.Fields.language, businessTokenVO.getLanguage());
        addHeader(mutate, RequestBasicInfo.Fields.oneId, businessTokenVO.getMerchantId());
        addHeader(mutate, RequestBasicInfo.Fields.userAccount, businessTokenVO.getMerchantAccount());
        addHeader(mutate, RequestBasicInfo.Fields.siteCode, businessTokenVO.getSiteCode());
        addHeader(mutate, RequestBasicInfo.Fields.deviceId, deviceId);
